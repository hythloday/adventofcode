...
object Day13 extends Advent {
  ...
  lazy val moods = input.map(parse(happiness, _).get).toMap
  lazy val arrangements = moods.keys.flatMap{ case (a, b) => List(a, b) }.toList.permutations.toList
  ...
  def part1 = arrangements.map(hScore).max
  def part2 = ???
